...
const ChargesPage = () => {
  ...
  const areas        = ['ALL', ...new Set(charges.map(c => c.district).filter(Boolean))];
  ...
  const filtered = charges.filter(c => {
    const matchArea    = selectedArea === 'ALL' || (c.district) === selectedArea;
    const matchSearch  = !searchTerm
      || c.chargeName.toLowerCase().includes(searchTerm.toLowerCase())
      || (c.city        || '').toLowerCase().includes(searchTerm.toLowerCase())
      || (c.ministerName  || '').toLowerCase().includes(searchTerm.toLowerCase());
                  ...
                  <div
                    ...
                    <div style={{ padding: '20px 24px 24px' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 16 }}>
                        {[
                          ['Pastor', charge.ministerName || '—'],
                          ['Members',  charge.memberCount  ? `${charge.memberCount}+ members` : '—'],
                          ['Service',  charge.serviceTimes || '—'],
