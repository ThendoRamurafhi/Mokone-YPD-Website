...
const AdminUsers = () => {
  ...
  const ROLE_COLOR = { ADMIN:{bg:'rgba(107,33,168,.1)',text:'#6b21a8'}, MEMBER:{bg:'rgba(37,96,64,.1)',text:'#1a4731'}, GUEST:{bg:'rgba(88,88,88,.1)',text:'#555'} };
    ...
    <AdminLayout>
      ...
      <div style={{ display:'flex', gap:12, marginBottom:20, flexWrap:'wrap' }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          {ROLES.map(r=>(
            <button key={r} onClick={()=>setFilter(r)} style={{ padding:'9px 16px', borderRadius:20, border:'1px solid rgba(26,71,49,.2)', background:filter===r?'#1a4731':'#fff', color:filter===r?'#fff':'#3d5247', fontFamily:"'Lato',sans-serif", fontSize:12, cursor:'pointer', transition:'all .2s' }}>{r}</button>
          ))}
        </div>
      </div>
      ...
      {loading ? <LoadingSpinner /> : (
        <div style={{ background:'#fff', borderRadius:12, border:'1px solid rgba(0,0,0,.07)', overflow:'hidden' }}>
          <table className="admin-table">
            ...
            <tbody>
              {filtered.length===0 ? <tr><td colSpan={5} style={{ textAlign:'center', padding:'40px', color:'#aaa' }}>No users found.</td></tr>
              : filtered.map(u=>(
                <tr key={u.userId}>
                  <td><div style={{ display:'flex', alignItems:'center', gap:12 }}>
                    ...
                    <div><div style={{ fontWeight:600, color:'#0d2b1a' }}>{u.firstName} {u.lastName}</div><div style={{ fontSize:12, color:'#aaa' }}>@{u.username}</div></div>
                  </div></td>
                  <td style={{ fontSize:13 }}>{u.email}</td>
                  <td>
                    <select value={u.role} onChange={e=>handleRoleChange(u.userId, e.target.value)}
                      style={{ padding:'5px 10px', borderRadius:5, border:'1px solid rgba(0,0,0,.15)', fontSize:12, color:ROLE_COLOR[u.role]?.text||'#555', background:'#fff', cursor:'pointer', fontFamily:"'Lato',sans-serif" }}>
                      {['ADMIN','MEMBER','GUEST'].map(r=><option key={r} value={r}>{r}</option>)}
                    </select>
                  </td>
                  <td style={{ fontSize:13 }}>{u.createdAt ? new Date(u.createdAt).toLocaleDateString('en-ZA') : '—'}</td>
                  <td>
                    <button onClick={()=>handleDelete(u)} style={{ background:'rgba(192,57,43,.1)', color:'#c0392b', border:'none', padding:'7px 14px', borderRadius:5, cursor:'pointer', fontSize:12, fontFamily:"'Lato',sans-serif", fontWeight:700 }}>Remove</button>
                  </td>
                </tr>
              ))}
            </tbody>
