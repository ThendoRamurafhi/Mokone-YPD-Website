...
const CAT_COLOR = {
  CONFERENCE:  { bg:'rgba(26,86,160,.12)',  text:'#1a56a0' },
  YOUTH:       { bg:'rgba(107,33,168,.12)', text:'#6b21a8' },
  COMMUNITY:   { bg:'rgba(26,102,64,.12)',  text:'#1a6640' },
  WORSHIP:     { bg:'rgba(146,64,14,.12)',  text:'#92400e' },
  EDUCATIONAL: { bg:'rgba(153,27,27,.12)',  text:'#991b1b' },
...
const CountBox = ({ value, label }) => (
  <div style={{
    ...
    padding:'18px 20px', minWidth:72,
  }}>
    <span style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:'clamp(2rem,5vw,3rem)', fontWeight:700, color:'#c9a84c', lineHeight:1 }}>
      {String(value).padStart(2, '0')}
    </span>
    <span style={{ fontFamily:"'Lato',sans-serif", fontSize:9, fontWeight:700, letterSpacing:'.18em', color:'rgba(255,255,255,.5)', marginTop:6, textTransform:'uppercase' }}>
      {label}
    </span>
  </div>
);
    ...
    <div style={{ fontFamily:"'Lato',sans-serif", paddingTop:64 }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .att-btn { width:44px; height:44px; border-radius:50%; border:2px solid rgba(26,71,49,.2); background:#fff; color:#1a4731; font-size:15px; font-weight:700; cursor:pointer; transition:all .2s; display:flex; align-items:center; justify-content:center; font-family:'Lato',sans-serif; }
        ...
        @media (max-width:768px) {
          ...
          .rsvp-side { position:static !important; }
        }
      `}</style>
      ...
      {/* ══ BODY ══ */}
      <div style={{ background:'#f7f9f7', padding:'60px 24px 80px' }}>
        <div className="ev-grid" style={{ maxWidth:1100, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 380px', gap:40, alignItems:'start' }}>
          ...
          <div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:12, marginBottom:32 }}>
              {[
                ...
                <div key={label} className="info-chip">
                  ...
                  <div>
                    <div style={{ fontSize:10, fontWeight:700, letterSpacing:'.12em', color:'#6b8070', marginBottom:2 }}>{label.toUpperCase()}</div>
            ...
            {event.maxAttendees && (
              <div style={{ background:'#fff', borderRadius:12, padding:'22px 24px', border:'1px solid rgba(0,0,0,.07)', marginBottom:24 }}>
                ...
                <div style={{ height:6, background:'rgba(26,71,49,.1)', borderRadius:3, overflow:'hidden' }}>
                  <div style={{ height:'100%', width:`${capacityPct}%`, background: capacityPct > 80 ? '#c0392b' : '#1a4731', borderRadius:3, transition:'width .6s' }} />
                </div>
                <div style={{ fontSize:12, color:'#6b8070', marginTop:8 }}>{event.currentAttendees || 0} of {event.maxAttendees} registered</div>
              </div>
            )}

            {event.description && (
              <div style={{ background:'#fff', borderRadius:12, padding:'28px', border:'1px solid rgba(0,0,0,.07)' }}>
                <h2 style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:24, fontWeight:700, color:'#0d2b1a', marginBottom:16 }}>About This Event</h2>
                <p style={{ fontSize:15, color:'#3d5247', lineHeight:1.9 }}>{event.description}</p>
              </div>
            )}
          </div>

          {/* RIGHT — RSVP */}
          <div className="rsvp-side" style={{ position:'sticky', top:88 }}>
            <div style={{ background:'#fff', borderRadius:14, border:'1px solid rgba(0,0,0,.07)', overflow:'hidden', boxShadow:'0 8px 32px rgba(26,71,49,.08)' }}>
              ...
              <div style={{ background:'linear-gradient(135deg,#1a4731,#256040)', padding:'24px 26px' }}>
                <div style={{ fontSize:10, fontWeight:700, letterSpacing:'.2em', color:'#c9a84c', marginBottom:8 }}>RSVP FOR THIS EVENT</div>
                <h3 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:19, color:'#fff', lineHeight:1.3 }}>{event.title}</h3>
                <p style={{ fontSize:12, color:'rgba(255,255,255,.55)', marginTop:6 }}>
              ...
              <div style={{ padding:'26px' }}>
                {rsvpDone ? (
                  <div style={{ textAlign:'center', padding:'20px 0' }}>
                    <div style={{ width:64, height:64, borderRadius:'50%', background:'rgba(26,102,64,.1)', border:'2px solid #1a6640', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px', fontSize:28, color:'#1a6640' }}>✓</div>
                    <h4 style={{ fontFamily:'Georgia,serif', fontSize:18, color:'#0d2b1a', marginBottom:8 }}>You're registered!</h4>
                    <p style={{ fontSize:13, color:'#6b8070', lineHeight:1.65 }}>
                      A confirmation has been sent to <strong>{form.email}</strong>. We look forward to seeing you!
                    </p>
                    <button onClick={() => { setRsvpDone(false); setForm({ name:'', email:'', attendees:1 }); }}
                      style={{ marginTop:20, background:'transparent', border:'1.5px solid #1a4731', color:'#1a4731', padding:'10px 22px', borderRadius:6, fontFamily:'Lato,sans-serif', fontSize:12, fontWeight:700, cursor:'pointer' }}>
                  ...
                  <div style={{ textAlign:'center', padding:'16px 0' }}>
                    <p style={{ color:'#6b8070', fontSize:14, lineHeight:1.6 }}>Registration for this event is now closed.</p>
                    <Link to="/events" style={{ display:'inline-block', marginTop:14, color:'#1a4731', fontWeight:700, fontSize:13 }}>Browse Upcoming Events →</Link>
                  </div>

                ) : (
                  <form onSubmit={submitRSVP}>
                    <div style={{ marginBottom:14 }}>
                      <label style={{ display:'block', fontSize:11, fontWeight:700, letterSpacing:'.12em', color:'#3d5247', marginBottom:7 }}>FULL NAME *</label>
                      <input className="det-input" type="text" required placeholder="Thendo Ramurafhi"
                        value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
                    </div>

                    <div style={{ marginBottom:14 }}>
                      <label style={{ display:'block', fontSize:11, fontWeight:700, letterSpacing:'.12em', color:'#3d5247', marginBottom:7 }}>EMAIL ADDRESS *</label>
                      <input className="det-input" type="email" required placeholder="you@example.com"
                        value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} />
                    </div>

                    <div style={{ marginBottom:20 }}>
                      <label style={{ display:'block', fontSize:11, fontWeight:700, letterSpacing:'.12em', color:'#3d5247', marginBottom:10 }}>NUMBER OF PEOPLE</label>
                    ...
                    {rsvpError && (
                      <div style={{ background:'rgba(192,57,43,.08)', border:'1px solid rgba(192,57,43,.2)', borderRadius:6, padding:'10px 14px', marginBottom:14 }}>
                        <p style={{ color:'#c0392b', fontSize:13, margin:0 }}>{rsvpError}</p>
                      </div>
                    )}

                    <button type="submit" disabled={rsvpLoading || spotsLeft === 0}
                      style={{ width:'100%', background:(rsvpLoading || spotsLeft === 0) ? '#ccc' : '#c9a84c', color:'#0d2b1a', border:'none', padding:'15px', borderRadius:7, fontFamily:'Lato,sans-serif', fontSize:13, fontWeight:700, cursor:(rsvpLoading || spotsLeft === 0) ? 'not-allowed' : 'pointer', letterSpacing:'.1em', transition:'background .2s' }}
                      ...
                      {rsvpLoading ? 'Reserving your spot...' : spotsLeft === 0 ? 'EVENT FULL' : `RSVP — ${form.attendees} PERSON${form.attendees > 1 ? 'S' : ''}`}
                    </button>

                    <p style={{ fontSize:11, color:'#aaa', textAlign:'center', marginTop:12 }}>
                      Free to attend · No account required
                    </p>
                  </form>
                )}
              </div>
            ...
            {/* Share */}
            <div style={{ marginTop:14, background:'#fff', borderRadius:12, padding:'18px 20px', border:'1px solid rgba(0,0,0,.07)' }}>
              <p style={{ fontSize:10, fontWeight:700, letterSpacing:'.14em', color:'#6b8070', marginBottom:12 }}>SHARE THIS EVENT</p>
              <div style={{ display:'flex', gap:8 }}>
                {[
                  ...
                  { label:'Copy Link', color:'#1a4731', fn: () => navigator.clipboard.writeText(window.location.href) },
                ].map(({ label, color, fn }) => (
                  <button key={label} onClick={fn}
                    style={{ flex:1, padding:'9px 4px', borderRadius:6, border:`1.5px solid ${color}33`, background:`${color}12`, color, fontSize:10, fontWeight:700, cursor:'pointer', fontFamily:'Lato,sans-serif', transition:'all .2s' }}
